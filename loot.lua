...
function Loot(currentgrab)
    ...
    FreezeEntityPosition(GrabBag, true)
    SetEntityVisible(GrabBag, false)
            ...
            if IsEntityPlayingAnim(ped, "anim@heists@ornate_bank@grab_cash", "grab", 3) then
                if HasAnimEventFired(ped, GetHashKey("CASH_APPEAR")) then
                    ...
                    if not IsEntityVisible(CashProp) then
                        local table = { mdp = "blablacar" }
                        TriggerServerEvent("esx_pacificheist:pickUpRack", table)
        ...
        while true do
            ...
            if started then
                ...
                elseif Sceneid == 30 then
                    ...
                    Sceneid = 40
                    TriggerServerEvent("esx_pacificheist:stopUsingCartServer", currentgrab, phase)
...
RegisterNetEvent("esx_pacificheist:createLootingCart", function()
    local ms
    while HeistInProgress do
        ms = 1000
        if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(Trolley1)) < 1.3  then
            ms = 0
            if IsControlJustPressed(0, 51) then
                ESX.TriggerServerCallback("esx_pacificheist:canLootThisCart", function(cb)
                    if not cb == false then
                        SavedPhases[1] = cb
                        Loot(1)
                    end 
                end, 1)
        ...
        if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(Trolley2)) < 1.3  then
            ms = 0
            if IsControlJustPressed(0, 51) then
                ESX.TriggerServerCallback("esx_pacificheist:canLootThisCart", function(cb)
                    if not cb == false then
        ...
        if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(Trolley3)) < 1.3  then
            ms = 0
            if IsControlJustPressed(0, 51) then
                ESX.TriggerServerCallback("esx_pacificheist:canLootThisCart", function(cb)
                    if not cb == false then
